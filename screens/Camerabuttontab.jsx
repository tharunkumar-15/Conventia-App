...
export default function Camerabuttontab({navigation}) {
  ...
  const responsedata=()=>{
    if(responseData.face=='Unknown')
    {
      ...
      ToastAndroid.show('Unknown Face is been detected',ToastAndroid.LONG,ToastAndroid.BOTTOM)
      //navigation.navigate("Home", { screen: 'UserPage' });
    }
    else if(responseData.face=='Face not found')
    {
      setLoading(false);
      ToastAndroid.show('Face not found',ToastAndroid.LONG,ToastAndroid.BOTTOM)
      //navigation.navigate("Home", { screen: 'UserPage' });
    }
    else
    {
  ...
  return (
    <View style={styles.camerabuttoncontainer}>
      ...
      <Text style={styles.pagetext}>Select the below options for Face Recognition</Text>
         <CustomButton buttonTitle='Open Camera' buttonStyle={{width:'65%',marginTop:25,backgroundColor:'#f95999'}} onPress={()=>takePhoto()}/>
         <CustomButton buttonTitle='Select File' buttonStyle={{width:'63%',marginTop:20,backgroundColor:'#f95999'}}/>
      </View>
      )}
    </View>
  )
